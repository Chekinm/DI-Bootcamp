...
async def start(message: types.Message):
    await message.answer("""Hi! I'm a bot. I can search yad2 around you and show you some flats you may intrested in. 
...
async def balcony(message: types.Message):
    ib = []
    ib.append(InlineKeyboardButton(text=f'YES',
                                  callback_data=f'{1}'))
    ib.append(InlineKeyboardButton(text=f'NO',
                                  callback_data=f'{0}'))
...
async def get(message: types.Message):
    ...
    """this function call get_request_data to get data from DB to pass it to search in yad2
    ...
    while ind < 6 and ind < len(resp_dict['feed']["feed_items"]): 
        ...
        try:
            ...
            print('Key error. Missed info for in yad2') 
        except IndexError: 
            print('Index error. Missed info for in yad2') 
